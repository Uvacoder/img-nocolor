...
import LikesOverlay from '../overlays/likesOverlay/LikesOverlay';
              ...
              <div key={item.id}>
                ...
                >
                  {item.href === '/account' || item.href === '/likes' ? (
                    <div className={router.pathname === item.href || (router.pathname === '/statistics' && item.href === '/account') || (router.pathname === '/profile' && item.href === '/account') ? 'underline underline-offset-4 cursor-default' : 'cursor-default'}>
                      {item.link}
                    </div>
                  ...
                  <div className={` ${item.href === '/account' && 'group-hover:flex'} hidden absolute h-20 justify-center items-center right-1/2 translate-x-1/2 bottom-1/2 -translate-y-1/2 pr-20 md:pr-0 w-[100rem] lg:w-[1000rem] bg-white/95 dark:bg-black/95 cursor-pointer gap-8`}>
                    {loginLinks.map((item) => (
                      <Link
                        key={item.id}
                        href={item.href}
